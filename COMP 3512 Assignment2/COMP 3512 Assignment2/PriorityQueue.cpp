...
#define LIST_ITERATOR(x, y) for(int x = PriorityLevel::CRITICAL; x != PriorityLevel::END; ++x) for(int y = 0; y < mPatientList[x].size(); ++y)  // ñÇñ@Ç≈Ç∑
...
PriorityQueue::PriorityQueue()
	: numOfPatient(0)
{
	mPatientList = new std::deque<Patient>[PriorityLevel::END - 1];	// Magic, èoóàÇÈÇæÇ©ÇÁ <-- Look chris, Moon runes
	// Empty
}
